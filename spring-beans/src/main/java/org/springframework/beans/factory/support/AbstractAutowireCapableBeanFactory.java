...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			//  这里只设置了一些标志位，在反射创建对象的时候，获取实例化策略，
		...
		try {
			...
			// 当存在动态代理的时候，会把除动态代理类之外，其他动态代理相关的对象创建完成，不知道哪个对象会被代理，第一个bean 创建的时候就需要准备好
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (mbd.isSingleton()) {
			// 如果是单例对象，从factoryBean 实例缓存中移除当前bean定义信息
		...
		// 下面要初始化了，最后的机会了，允许beanPostProcesser 去修改合并的beanDefiation
		...
		// 判断是否需要提前曝光，这个地方就是解决 循环依赖（实例化完成后，要进行初始化赋值操作，赋值的类型可能是引用类型，
		// 需要从容器中获取，此时被依赖的对象可能存在，也可能不存在，此对象内部再次依赖当前对象的时候，就产生了循环依赖）
		// Eagerly cache singletons to be able to resolve circular references
		// even when triggered by lifecycle interfaces like BeanFactoryAware.
		// 初始化阶段，只是填充属性，不会改变对象，但是有例外
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
				// 一般用于设置属性,如果返回false,这个地方就直接返回，后面不再对属性操作
		...
		// 封装了属性值，例如xml 里Bean标签内的property标签
		...
		// 这个地方就是Bean 属性解析的地方，解析属性上的注解@Autowired等等，主要的两个解析类就是 AutowiredAnnotationBeanPostProcessor、CommonAnnotationBeanPostProcessor
		// 自定义注解识别类信息，就可以这么做
		if (hasInstAwareBpps) {
			if (pvs == null) {
		...
		if (pvs != null) {
			//应用给定的属性，解决任何在这个bean工厂运行时其他bean的引用，必须使用深拷贝，所以我们不会用永久修改这个
	...
	protected void autowireByName(
		...
		for (String propertyName : propertyNames) {
			// 如果该bean工厂有propertyName的beanDefinition 或者外部注册的singleton实例
			if (containsBean(propertyName)) {
				// 从bean工厂获取该对象。内部找不到就去创建
				Object bean = getBean(propertyName);
				pvs.add(propertyName, bean);
				// 注册propertyName 和beanName 的依赖关系
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		...
		for (PropertyDescriptor pd : pds) {
			...
			if (pd.getWriteMethod() != null // 是都存在写入方法 即属性的set方法
					&& !isExcludedFromDependencyCheck(pd) // pd 是否被排除在依赖检查之外
					&& !pvs.contains(pd.getName()) &&     // pvs 是否包含pd的属性名
					!BeanUtils.isSimpleProperty(pd.getPropertyType())) {  // pd的属性值是不是简单值类型
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		// 下面就是很复杂的地方，主要是要考虑到各种细节，一个类里面有可能定义各种类型的变量
